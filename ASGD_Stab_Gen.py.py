...
# Distributes the dataset subsets to workers.
# Crucially, it ensures that the specific "changed sample" (which distinguishes S from S')
# is assigned to a specific worker (Worker 0), while other samples are distributed normally.
# This allows precise tracking of how that single change propagates.
...
def distribute_data(n_workers, neighbor_datasets_dict, heter=True, seed=None):
    ...
    main_subset = neighbor_datasets_dict[0]
    total_len = len(main_subset)
    
    changed_indices_positions = set() 

    for k in range(1, len(neighbor_datasets_dict)):
        subset_k = neighbor_datasets_dict[k]
        for pos, (val0, valk) in enumerate(zip(main_subset, subset_k)):
            if val0 != valk:
                changed_indices_positions.add(pos)
    
    changed_pos_list = list(changed_indices_positions)

    all_positions = np.arange(total_len)
    unchanged_mask = np.ones(total_len, dtype=bool)
    unchanged_mask[changed_pos_list] = False
    unchanged_positions = all_positions[unchanged_mask]
    
    np.random.shuffle(unchanged_positions)
    
    chunk_size = total_len // n_workers
    
    if len(changed_pos_list) > chunk_size:
        raise ValueError(f"Too many changed samples ({len(changed_pos_list)}) for one worker (capacity {chunk_size})!")

    target_worker_id = 0
    final_indices_map = np.concatenate([np.array(changed_pos_list), unchanged_positions])
    
    chunk_size = total_len // n_workers
    
    for key, subset in neighbor_datasets_dict.items():
        shuffled_subset = [subset[i] for i in final_indices_map]
        distributed_subset = {}
        for worker_id in range(n_workers):
            start_idx = worker_id * chunk_size
            end_idx = (worker_id + 1) * chunk_size if worker_id != n_workers - 1 else total_len
        ...
        if key != 0:
            for worker_id in range(n_workers):
                s0 = set(distributed[0][worker_id])
                sk = set(distributed[key][worker_id])
                diff = s0.symmetric_difference(sk)
                if len(diff) > 0:
                    changed_workers.append((key, worker_id))

    print(f"  [Info] All {len(changed_pos_list)} changed samples assigned to Worker {target_worker_id}.")
    
    return distributed, changed_workers
...
class Logger:
    ...
    def update(self, iteration, delay, NMT, t_loss, t_acc, te_loss, te_acc, gap, stab, delay_accum):
        with open(self.file_path, 'a') as f:
            f.write(f"Iter: {iteration}, Delay: {delay}, NMT: {NMT}, "
                    f"TrLoss: {t_loss:.4f}, TrAcc: {t_acc:.4f}, "
                    f"TeLoss: {te_loss:.4f}, TeAcc: {te_acc:.4f}, "
                    f"Gap: {gap:.4f}, Stab: {stab:.6f}, DelayAccum: {delay_accum:.4f}",
                    )
            f.write("\n")
...
# The Parameter Server Actor.
# It maintains the global model state (and neighbor models).
# Workers push gradients here, and this actor updates the weights.
# It also handles logging and evaluation.
@ray.remote
class RayServer:
    def __init__(self, args, distributed_info, changed_workers, log_paths):
        if args.device == 'cpu':
            torch.set_num_threads(1)
        # init params
        self.args = args
        self.distributed_datasets = distributed_info
        self.changed_workers = changed_workers
        ...
        self.history = {"iterations":[], "stability":[]}
    ...
    def get_weights(self):
        ...
        # Returns the current model weights to workers.
        # Transferred via CPU to avoid serialization overhead and reduce GPU memory usage.
        weights = {i: {k: v.clone() for k, v in self.models[i].state_dict().items()} for i in self.models}
        # In ASGD modes, return directly to workers without ray.put if we pull via handle, but here we can keep ray.put for standard backward compat, 
        # or we just return the dict to match random SGD
        return weights, self.iteration
    ...
    # The core asynchronous update step.
    # 1. Calculates 'delay' (staleness) of the gradients.
    # 2. Updates the model weights using the gradients.
    # 3. Periodically triggers evaluation (calculation of stability, loss, accuracy).
    def apply_gradients(self, worker_id, pull_iter, grads_dict):
        ...
        if len(self.worker_buffer) >= b:
            ...
            if self.args.lr_type == 'adaptive':
                ...
                # delay=0 -> factor=1.0; delay=1 -> factor=0.5
                scale_factor = (1/self.args.lr) / (self.args.ada_fac*avg_delay+1/self.args.lr)
    ...
    # - Calculates 'Stability': Euclidean distance between the main model and neighbor models.
    # - Calculates Training/Test Loss and Accuracy.
    ...
    def _evaluate_routine(self, current_iter, delay):
        ...
        print(f"Iter:{current_iter}| Delay:{delay}| NMT:{NMT}| Stab:{stability:.6f}| Train Loss:{tr_loss:.6f}| Train Acc:{tr_acc:.6f}| Test Loss:{te_loss:.6f}| Test Acc:{te_acc:.6f}| Gap:{gen_gap:.6f}| DelayAccum:{delay_accum:.4f}")

        # Logging
        self.logger.update(current_iter, delay, NMT, tr_loss, tr_acc, te_loss, te_acc, gen_gap, stability, delay_accum)
    ...
    def _run_inference(self, model, dataset):
        model.eval()
        # Create temp loader
        loader = DataLoader(dataset, batch_size=1000, shuffle=False, num_workers=4 if self.device == 'cuda' else 0) # Large batch for faster inference
        total_loss = 0
        correct = 0
        total = 0
        with torch.no_grad():
...
class RayWorker:
    ...
    def _compute_and_report(self, pull_iter):
        ...
        for i in range(self.num_models):
            ...
            # [Synchronization Point] Get data based on shared position indices
            # If this position is not a Changed Sample, the data obtained is exactly the same as Model 0
        ...
        # --- 4. Push Gradients ---
        finished = ray.get(self.server.apply_gradients.remote(self.worker_id, pull_iter, grads_dict))
        return finished

    def accept_weights_and_compute(self, weights_dict, iteration):
        """
        Receive weights from server, update local buffer, 
...
# [Explanation]
# This block orchestrates the entire experiment.
# 1. Parses command line arguments.
# 2. Initializes the Ray cluster.
# 3. Iterates through experiment configurations (repeats, worker counts).
# 4. Prepares data indices (splitting data for workers).
# 5. Launches Server and Worker actors.
# 6. Monitors progress and shuts down actors upon completion.
if __name__ == "__main__":
    ...
    parser.add_argument("--ada-fac", type=float, default=10.0, help="Adaptive LR factor for delay scaling")
    ...
    for r in range(1, args.repeats + 1):
        ...
        for nworkers in num_workers_list:
            ...
            # for lr in lr_list:
                ...
                # Load dataset just to get length and create indices
                temp_data = load_data(args.dataset, args.dataset_path, 'train')
                total_req = args.num_samples * args.num_workers
                neighbor_datasets = create_neibordataset(temp_data, total_req, args.n_pairs, seed=current_seed)
                
                distributed_datasets, changed_workers = distribute_data(
                    nworkers, neighbor_datasets, heter=True, seed=current_seed
                )
                ...
                server_actor = RayServer.options(num_cpus=1).remote(
                    args, distributed_datasets, changed_workers, log_paths
                )
                ...
                for i in range(args.num_workers):
                    w_indices = {
                        k: distributed_datasets[k][i] for k in range(args.n_pairs + 1)
                    }
                    ...
                    if futures:
                        ...
                        if len(not_done) == 0:
                            ...
                            try:
                                ...
                                print(f"[ERROR] Workers crashed with error:\n{e}")
                            break
